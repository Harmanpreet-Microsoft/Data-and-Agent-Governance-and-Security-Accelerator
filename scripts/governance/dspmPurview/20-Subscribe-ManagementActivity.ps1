...
foreach($ct in $spec.activityExport.contentTypes){ try{ Invoke-RestMethod -Method POST -Uri "$base/start?contentType=$ct" -Headers $h -ErrorAction SilentlyContinue | Out-Null }catch{} }
